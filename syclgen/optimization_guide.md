...
### 1.2 Sub-Group Size
- **Do NOT** specify `[[intel::reqd_sub_group_size()]]` by default — let the compiler
  choose the optimal sub-group size. Forcing a sub-group size on kernels that don't
  need it (e.g., simple copy/elementwise kernels) causes performance regressions.
- **Only** use `[[intel::reqd_sub_group_size(16)]]` when the kernel **depends** on a
  specific sub-group size, such as:
  - Sub-group collective operations (shuffles, broadcasts, reductions) where the
    algorithm is written for a fixed sub-group width.
  - Joint matrix (XMX) operations that require a specific sub-group size.
  - Hand-tuned reductions or scans that assume a particular sub-group layout.
- SIMD16 is the native width for Xe2, but the compiler may select SIMD32 or SIMD8
  when that yields better performance for a given kernel.
- Sub-group operations (shuffles, broadcasts, reductions) are single-cycle on Intel GPUs.

```cpp
// GOOD: kernel uses sub-group reductions → specify sub-group size
q.parallel_for(nd_range<1>({N}, {256}),
  [=](nd_item<1> item) [[intel::reqd_sub_group_size(16)]] {
    auto sg = item.get_sub_group();
    float val = sycl::reduce_over_group(sg, x, sycl::plus<float>());
  });

// GOOD: simple kernel → let compiler choose sub-group size
q.parallel_for(nd_range<1>({N}, {256}),
  [=](nd_item<1> item) {
    out[item.get_global_id(0)] = in[item.get_global_id(0)];
  });
```
...
- [ ] `[[intel::reqd_sub_group_size()]]` only used when kernel logic requires it
