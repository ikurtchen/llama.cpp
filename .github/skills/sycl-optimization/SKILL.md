...
3. VALIDATE  run.sh build -> evidence.sh record test (correctness is a HARD GATE)
4. MEASURE   evidence.sh record bench on the KERNEL BENCHMARK across its shape set (warm-up + iters)
5. DECIDE    faster AND correct -> git commit (KEEP); else git revert (REJECT + log root cause)
6. RECORD    append the trial + its evidence run ids to optimization/<id>.json; regen PROGRESS.md
7. REPEAT    next hypothesis, until a stop criterion fires
8. CLOSE     deep-profile the best commit into opt_<id>/final/, then evidence.sh diagnosis <id>
```
...
**Then check the diagnosis against the outcome.** Once `final/` exists:
```bash
.sycl/scripts/evidence.sh diagnosis <id>      # -> .sycl/reports/opt_<id>/DIAGNOSIS.md
```
This reads the before/after counter CSVs and places them beside the bottleneck you claimed, then
issues a verdict:
- **`supported`** — the blamed metric moved in the predicted direction and the kernel got faster. The
  explanation survives contact with the counters, so it is safe to generalise into a lesson.
- **`unexplained`** — the kernel got faster but the blamed metric did not move (or moved the wrong
  way). The **win is still real** — it is gated on measurement, not on the story — but the *reason* is
  wrong. Re-diagnose before repeating the change on another kernel or writing it into `lessons.md`; a
  wrong causal story is how a one-off win becomes a rule that quietly costs performance elsewhere.
- **`unchecked`** — no `dominant_stall` was recorded, so nothing could be falsified. Fix by recording
  the diagnosis, not by ignoring the verdict.

Store the generated path in `diagnosis.artifacts.diagnosis_md`. A kernel with kept trials but empty
`baseline/` or `final/` fails `evidence.sh verify` (check E6): without both, "it got faster because X"
is unfalsifiable.
...
- **A kept trial must be backed by records.** Capture the correctness run and the before/after
  benchmark of every trial you KEEP and store their run ids in `trials[].evidence`
  (`test`, `bench_before`, `bench_after`):
  ```bash
  .sycl/scripts/evidence.sh record test  <id> --label trial-<n> -- "<test cmd>"
  .sycl/scripts/evidence.sh record bench <id> --label trial-<n>-after --no-gate -- "<bench cmd>"
  ```
  Label the kernel's first benchmark `--label baseline` and its last `--label final`, and point
  `performance.evidence.{baseline,optimized}` at them. Both must share the same env fingerprint or the
  "speedup" is comparing two machines (check E7). A kept trial missing its records fails check E3 —
  which is the point: an improvement nobody can re-measure is not an improvement anyone can ship.
...
- Evidence: baseline/final bench run ids and the `DIAGNOSIS.md` verdict
  (`supported` | `unexplained` | `unchecked`).
