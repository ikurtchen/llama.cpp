...
- **No claim without a record.** The unit-test pass you write into state must come from a run captured
  with `.sycl/scripts/evidence.sh record test`, and its run id must be stored in `unit_test.evidence`.
  Never write `result: "pass"` from memory, from a run you did not capture, or from a partially-read
  log — see `.sycl/references/evidence-model.md`.
...
**Capture the passing run as evidence.** Iterate with plain `run.sh` while debugging; once it is
green, run it **once more** through `evidence.sh` so the pass is a record rather than a claim:
```bash
.sycl/scripts/evidence.sh record test <id> --label migrate \
    --reference "cpu-oracle fp64 accum, dev/common.hpp:validate_result" \
    --tolerance "rtol=1e-5,atol=1e-6" --tolerance-source "dev/cuda/<id>.cu" \
    --shape "B=8,T=1024,C=768" --dtype fp32 --seed 1337 \
    -- "./dev/sycl/<id>"
```
It keeps the raw output, hashes it, and stores the exit code, commit and machine fingerprint. Note
the printed `run_id`. Where the harness prints them, the measured `max abs/rel err` and the
`rtol`/`atol` are pulled out of the log automatically — which is what turns "PASS" into "PASS with
3× margin", the difference between a reviewer trusting the result and being able to check it.
Record the **tolerance source** honestly: a tolerance you chose yourself is a much weaker claim than
one inherited from the CUDA harness, and the field is there so the reviewer can tell them apart.
...
- Set `unit_test.evidence` to the `run_id` from step 5, **in the same edit that writes
  `unit_test.result`**. A `pass` with no record fails `evidence.sh verify` (check E1) — the point is
  that an unbacked pass shows up as *missing*, not as a pass.
- Update `.sycl/state/kernels/index.json` status.
- **Measure the migrated code volume**: `bash .sycl/scripts/loc.sh kernel <id>`. It recounts the
  original kernel spans and the SYCL you just wrote (from `sycl_impl` — so list **every** SYCL file,
  comma-separated, and the same-named originals `dev/cuda/<k>.cu` are picked up automatically by the
  mirror rule). If the port covers sources the kernel's `source` field does not name, add them to
  `loc.source_spec` before re-running; check `loc.unresolved` is empty. This LOC lands in the final
  report as "how much CUDA/Triton was migrated", so a missing file here understates the work.
...
- Unit test: pass/fail + tolerance; reference kind used; **evidence run id**.
- **LOC**: `<source code lines> → <SYCL code lines>` (from the `loc` block).
