...
- **Write `source` as `<file.py>:<kernel_fn>`** (the exact `@triton.jit` function name). Step 7
  resolves it to a line span, so a vague `source` costs you the LOC figure.
...
### 7. Measure LOC (how much Triton this migration will actually cover)
The final report states how many lines of Triton were migrated, so capture it here — while the source
symbols are fresh:
```bash
...
```
It resolves each `def <kernel_fn>` (decorators included) to its indentation-delimited span and counts
**code lines only** (comments, docstrings and blanks excluded). Then:
- Check `loc.unresolved` in each detail file; fix the symbol name in `source`, or add explicit specs
  to `loc.source_spec` (`"ops/rms.py"`, `"ops/rms.py:40-120"`, `"ops/rms.py:fn1,fn2"`). Hand-added
  specs survive re-measurement.
- Add the **Python launcher/wrapper** (grid construction, `autograd.Function`) to `loc.source_spec`
  when the port has to reproduce it — otherwise the report understates the work.
- Re-run `loc.sh all` after `migrate` to fill in the SYCL side; the rollup in `.sycl/state/loc.json`
  merges overlapping spans per file so a module holding several kernels is never double-counted.
...
- **Migration size**: total Triton code lines covered and its share of the project's Triton (from
