...
runner (.sycl/scripts/run.sh). Concurrent GPU work is forbidden: run.sh serializes build/test/profile/bench/exec.
---
...
**Fingerprint the machine before measuring anything.**
```bash
.sycl/scripts/evidence.sh env      # -> env-<sha8>, store in project.json env.evidence
```
This snapshots the toolchain, GPU, platform/arch and the `freq_pinned` flag, and stamps every later
test/bench/e2e record with the resulting id. It exists so that a baseline→final comparison can be
**checked** rather than assumed: two numbers measured under different fingerprints are not a speedup,
and `evidence.sh verify` fails that case (check E7) instead of letting it into the report. Unpinned
clocks likewise surface as an `unstable` warning against every benchmark built on them.
...
- **Capture every benchmark you intend to cite.** Wrap the invocation so the raw output, the exact
  command, the commit and the machine fingerprint are kept beside the number:
  ```bash
  .sycl/scripts/evidence.sh record bench <kernel-id> --label baseline \
      --shape "B=8,T=1024,C=768" --shape-class model --dtype fp32 \
      --warmup 3 --iters 20 --no-gate \
      -- "bash framework/skills/sycl-profiler/scripts/bench.sh ./bench_<id> ..."
  ```
  `bench.sh`'s JSON line is parsed automatically into `median_ms`/`p90_ms`/`stddev_ms`/`min_ms` and
  `GB/s`/`TFLOP/s` are read from the log. Use `--no-gate` for benchmarks: a benchmark that runs is
  `recorded`, not `pass`/`fail`. **Store the whole sample vector, not just the median** — that is what
  lets a reviewer recompute the statistic and see whether a 4% win is outside the run-to-run spread.
  Put the printed run id in the kernel's `performance.evidence.baseline` / `.optimized`.
...
**Evidence.** Capture the workload run itself (`evidence.sh record e2e baseline|final`, and
`--label case-<name>` per e2e case) and store the ids in `e2e.json` / the baseline+final snapshots.
Also set `method` to how wall-clock was attributed — `unitrace device-timing`, `unitrace slice`, or
`in-app sycl::event timers`. These differ in precision and in what they include, and a slice traces
only part of the run; naming the method keeps the report from presenting a sampled estimate as an
exact measurement, and tells a reviewer whether a small share is signal or noise.
...
- Every number you write into state carries its `evidence` run id. `.sycl/reports/` is pulled back
  from the remote runner automatically, so evidence captured on the GPU host is auditable locally.
