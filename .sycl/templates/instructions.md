...
### Engine behavior guardrails
<!-- These are toggled in .sycl/config.json under `guardrails`, NOT here — they are per-engine safety
     switches, not project overrides. Enable them for LLM engines that misbehave:
       gpu_hang_recovery  = on a GPU hang, only kill the process; never reset/reload the GPU driver
                            (stop + report if it won't die).
       no_lazy_completion = never skip kernels or stop early for difficulty; finish the whole workflow.
     Left here only as a pointer; edit config.json to change them. -->
- See `.sycl/config.json` → `guardrails` for the GPU-hang and no-lazy-completion safety switches.
