...
PHASES="detect inventory migrate integrate profile-e2e optimize done report"
...
cmd_import() {
  ...
  #     "note": "..." }   -- every field optional (phase defaults to "import", ts to now).
...
with open(events, "a") as f:
    for r in rows:
        ...
        n += 1
print(f"imported {n} usage row(s) from {src}")
PY
  cmd_rollup
}
