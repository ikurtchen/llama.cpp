...
static void group_norm_f32(const float* x, float* dst, const int group_size, const int ne_elements, const float eps,
    ...
    if (block_size > WARP_SIZE) {
        ...
        // Sum partials across warps using SLM
        if (lane_id == 0) {
            s_sum[warp_id] = tmp;
        }
        item_ct1.barrier(sycl::access::fence_space::local_space);
...
static void norm_f32_sycl(const float * x, float * dst, const int ncols, const int nrows, const int nchannels, const int nsamples,
    ...
    // Optimization: Use 256 threads for small ncols (< 1024) instead of WARP_SIZE (16)
    // This provides better parallelism for the reduction while fitting in Xe2 SLM (64KB)
    // With 256 threads: 256/16=16 warps, need 16 float2 partials = 128 bytes SLM
    if (ncols < 1024) {
...
static void rms_norm_f32_sycl(const float* x, float* dst, const int ncols, const int nrows, const int nchannels, const int nsamples,
    ...
    // Optimization: Use 256 threads for small ncols (< 1024) instead of WARP_SIZE (16)
    // This provides better parallelism for the reduction while fitting in Xe2 SLM (64KB)
    // With 256 threads: 256/16=16 warps, need 16 float partials = 64 bytes SLM
    if (ncols < 1024) {
        const int work_group_size = 256;
        const sycl::range<3> block_dims(1, 1, work_group_size);
        stream->submit([&](sycl::handler& cgh) {
