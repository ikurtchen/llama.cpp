#include "opt_step_adamw.hpp"
#include "common.hpp"
#include <cmath>

#define SYCL_OPT_STEP_ADAMW_BLOCK_SIZE 256

static void opt_step_adamw_kernel(float * x, const float * g, float * g_m, float * g_v, const float * pars,
                                  const int64_t k, const sycl::nd_item<1> & item) {
...
void ggml_sycl_opt_step_adamw(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    const int64_t num_blocks = (ne + SYCL_OPT_STEP_ADAMW_BLOCK_SIZE - 1) / SYCL_OPT_STEP_ADAMW_BLOCK_SIZE;
    ...
    stream->parallel_for(
        ...
        [=](sycl::nd_item<1> item) {
            opt_step_adamw_kernel(src0_d, src0_grad_d, src0_grad_m_d, src0_grad_v_d, adamw_params_d, ne, item);
        });
}