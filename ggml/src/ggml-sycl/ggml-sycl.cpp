...
static void ggml_sycl_conv_2d_dw(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    scope_op_debug_print scope_dbg_print(__func__, dst, /*num_src=*/2);
    ggml_sycl_op_conv_2d_dw(ctx, dst);
}

static void ggml_sycl_conv_transpose_2d(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    scope_op_debug_print scope_dbg_print(__func__, dst, /*num_src=*/2);
    ggml_sycl_op_conv_transpose_2d(ctx, dst);
}
...
static bool ggml_sycl_compute_forward(ggml_backend_sycl_context & ctx, struct ggml_tensor * dst) try {
    ...
    switch (dst->op) {
        ...
        case GGML_OP_CONV_TRANSPOSE_2D:
            ggml_sycl_conv_transpose_2d(ctx, dst);
            break;
...
static bool ggml_sycl_topk_moe_fusion(const struct ggml_cgraph * cgraph, int node_idx, ggml_sycl_topk_moe_args & args) {
    args.sigmoid         = false;
    args.softmax         = false;
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (!disable_fusion) {
            ...
            if (cgraph->nodes[i]->op == GGML_OP_UNARY || cgraph->nodes[i]->op == GGML_OP_SOFT_MAX ||
                ...
                const bool can_fuse = ggml_sycl_topk_moe_fusion(cgraph, i, args);
                ...
                if (can_fuse) {
                    ...
                    if (!args.delayed_softmax) {
                        ...
                        if (ggml_can_fuse_subgraph(cgraph, i, ops.size(), ops.data(), out_nodes, 2) &&
                                ggml_sycl_should_use_topk_moe(node, logits, weights, ids)) {
                            ggml_sycl_op_topk_moe(*sycl_ctx, logits, weights, ids, clamp, scale, bias, args);
                            i += ops.size() - 1;
                            continue;
                        }
                        ...
                        if (ggml_can_fuse_subgraph(cgraph, i, ops.size(), ops.data(), out_nodes, 2) &&
                                ggml_sycl_should_use_topk_moe(softmax, logits, weights, ids)) {
                            ggml_sycl_op_topk_moe(*sycl_ctx, logits, weights, ids, clamp, scale, bias, args);
                            i += ops.size() - 1;
                            continue;
                        }
