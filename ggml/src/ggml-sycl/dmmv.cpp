...
#ifndef GGML_SYCL_DMMV_WG_SIZE
#define GGML_SYCL_DMMV_WG_SIZE 256
#endif

#ifndef GGML_SYCL_DMMV_K_WG_SIZE
#define GGML_SYCL_DMMV_K_WG_SIZE 256
#endif

#ifndef GGML_SYCL_DMMV_USE_SLM
#define GGML_SYCL_DMMV_USE_SLM 1
#endif

// Intel Xe2 (Arc Pro B60) native SIMD width is 16 (SIMD16).
// Using SIMD32 forces non-native mode, which halves occupancy and reduces performance by 15-40%.
// See: hw_spec (Intel Xe2 architecture) and optimization_guide (Intel DPC++ Compiler Optimization Reference)
// Set GGML_SYCL_DMMV_K_SUBGRP_SIZE=32 to force SIMD32 (original behavior) if needed for compatibility.
#ifndef GGML_SYCL_DMMV_K_SUBGRP_SIZE
#define GGML_SYCL_DMMV_K_SUBGRP_SIZE WARP_SIZE
#endif
...
static void dequantize_mul_mat_vec_dispatch(const void *vx, const dfloat *y,
        ...
        stream->submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                    dequantize_mul_mat_vec_slm<QK, QR, dequantize_kernel>(
                        vx, y, dst, ncols, nrows, item_ct1, partial_sums);
                });
        });
    }
#else
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, WARP_SIZE);
    {
        ...
        stream->parallel_for(
            sycl::nd_range<3>(block_nums * block_dims, block_dims),
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                dequantize_mul_mat_vec<QK, QR, dequantize_kernel>(
...
static void dequantize_mul_mat_vec_slm(const void * __restrict__ vx, const dfloat * __restrict__ y, float * __restrict__ dst, const int ncols, const int nrows,
    ...
    const int subgrp_id = item_ct1.get_sub_group().get_group_id()[0];
    ...
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, GGML_SYCL_DMMV_WG_SIZE);
    {
        ...
        stream->submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for(
                sycl::nd_range<3>(block_nums * block_dims, block_dims),
                [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                    dequantize_mul_mat_vec_slm<1, 1, convert_f16>(vx, y, dst, ncols,
                                                              nrows, item_ct1, partial_sums);
                });
        });
    }
#else
...
static void dequantize_mul_mat_vec_q2_k(const void *__restrict__ vx,
    ...
    // sum up partial sums and write back result
#pragma unroll
    for (int mask = GGML_SYCL_DMMV_K_SUBGRP_SIZE / 2; mask > 0; mask >>= 1) {
        tmp +=
            dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), tmp, mask);
    }
...
#if GGML_SYCL_DMMV_USE_SLM
static void dequantize_mul_mat_vec_q3_k_slm(const void *__restrict__ vx,
                                        const float *__restrict__ yy,
                                        float *__restrict__ dst,
                                        const int ncols, int nrows,
                                        const sycl::nd_item<3> &item_ct1,
...
#pragma unroll
    ...
    if (item_ct1.get_local_id(2) == 0) {
        float result = 0.0f;
        for (int i = 0; i < DMMV_K_SUBGRP_CNT; i++) {
            result += partial_sums[i];
        }
        dst[row] = result;
    }
...
#if GGML_SYCL_DMMV_USE_SLM
static void dequantize_mul_mat_vec_q4_k_slm(const void *__restrict__ vx,
                                        const float *__restrict__ yy,
                                        float *__restrict__ dst,
                                        const int ncols, int nrows,
                                        const sycl::nd_item<3> &item_ct1,
...
static void dequantize_mul_mat_vec_q2_k_slm(const void *__restrict__ vx,
                                        const float *__restrict__ yy,
                                        float *__restrict__ dst,
                                        const int ncols, int nrows,
                                        const sycl::nd_item<3> &item_ct1,
...
#if GGML_SYCL_DMMV_USE_SLM
static void dequantize_mul_mat_vec_q5_k_slm(const void *__restrict__ vx,
                                        const float *__restrict__ yy,
                                        float *__restrict__ dst,
                                        const int ncols,
                                        const sycl::nd_item<3> &item_ct1,
...
static void dequantize_mul_mat_vec_q4_0_sycl(const void *vx, const dfloat *y,
                                             ...
                                             const int nrows,
                                             dpct::queue_ptr stream) {
    dequantize_mul_mat_vec_dispatch<QK4_0, QR4_0, dequantize_q4_0>(vx, y, dst, ncols, nrows, stream);
}
...
static void dequantize_mul_mat_vec_q4_1_sycl(const void *vx, const dfloat *y,
                                             ...
                                             const int nrows,
                                             dpct::queue_ptr stream) {
    dequantize_mul_mat_vec_dispatch<QK4_1, QR4_1, dequantize_q4_1>(vx, y, dst, ncols, nrows, stream);
}
...
static void dequantize_mul_mat_vec_q5_0_sycl(const void *vx, const dfloat *y,
                                             ...
                                             const int nrows,
                                             dpct::queue_ptr stream) {
    dequantize_mul_mat_vec_dispatch<QK5_0, QR5_0, dequantize_q5_0>(vx, y, dst, ncols, nrows, stream);
}
...
static void dequantize_mul_mat_vec_q5_1_sycl(const void *vx, const dfloat *y,
                                             ...
                                             const int nrows,
                                             dpct::queue_ptr stream) {
    dequantize_mul_mat_vec_dispatch<QK5_1, QR5_1, dequantize_q5_1>(vx, y, dst, ncols, nrows, stream);
}
...
static void dequantize_mul_mat_vec_q8_0_sycl(const void *vx, const dfloat *y,
                                             ...
                                             const int nrows,
                                             dpct::queue_ptr stream) {
    dequantize_mul_mat_vec_dispatch<QK8_0, QR8_0, dequantize_q8_0>(vx, y, dst, ncols, nrows, stream);
}
...
static void dequantize_mul_mat_vec_q2_K_sycl(const void *vx, const float *y,
    ...
    const sycl::range<3> block_nums(1, 1, block_num_y);
#if GGML_SYCL_DMMV_USE_SLM
    const sycl::range<3> block_dims(1, ny, GGML_SYCL_DMMV_K_WG_SIZE);
    stream->submit([&](sycl::handler& cgh) {
        sycl::local_accessor<float, 1> partial_sums(sycl::range<1>(DMMV_K_SUBGRP_CNT), cgh);

        cgh.parallel_for(
            sycl::nd_range<3>(block_nums * block_dims, block_dims),
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
                dequantize_mul_mat_vec_q2_k_slm(vx, y, dst, ncols, nrows, item_ct1, partial_sums);
            });
    });
#else
    const sycl::range<3> block_dims(1, ny, GGML_SYCL_DMMV_K_SUBGRP_SIZE);
    stream->parallel_for(
        sycl::nd_range<3>(block_nums * block_dims, block_dims),
        [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
...
static void dequantize_mul_mat_vec_q3_K_sycl(const void *vx, const float *y,
    ...
    stream->submit([&](sycl::handler& cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(block_nums * block_dims, block_dims),
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
                dequantize_mul_mat_vec_q3_k_slm(vx, y, dst, ncols, nrows, item_ct1, partial_sums);
            });
    });
#else
    const sycl::range<3> block_dims(1, ny, GGML_SYCL_DMMV_K_SUBGRP_SIZE);
...
static void dequantize_mul_mat_vec_q4_K_sycl(const void *vx, const float *y,
    ...
    stream->submit([&](sycl::handler& cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(block_nums * block_dims, block_dims),
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
                dequantize_mul_mat_vec_q4_k_slm(vx, y, dst, ncols, nrows, item_ct1, partial_sums);
            });
    });
#else
    const sycl::range<3> block_dims(1, ny, GGML_SYCL_DMMV_K_SUBGRP_SIZE);
...
static void dequantize_mul_mat_vec_q5_K_sycl(const void *vx, const float *y,
    ...
    GGML_ASSERT(ncols % QK_K == 0);
#if GGML_SYCL_DMMV_USE_SLM
    const sycl::range<3> block_dims(1, 1, GGML_SYCL_DMMV_K_WG_SIZE);
    stream->submit([&](sycl::handler& cgh) {
        sycl::local_accessor<float, 1> partial_sums(sycl::range<1>(DMMV_K_SUBGRP_CNT), cgh);
        ...
        cgh.parallel_for(
            ...
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
                dequantize_mul_mat_vec_q5_k_slm(vx, y, dst, ncols, item_ct1, partial_sums);
            });
    });
#else
    const sycl::range<3> block_dims(1, 1, GGML_SYCL_DMMV_K_SUBGRP_SIZE);
    stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, nrows) * block_dims, block_dims),
        [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
...
static void dequantize_mul_mat_vec_q6_K_sycl(const void *vx, const float *y,
    ...
    stream->submit([&](sycl::handler& cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(block_nums * block_dims, block_dims),
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(GGML_SYCL_DMMV_K_SUBGRP_SIZE)]] {
                dequantize_mul_mat_vec_q6_k_slm(vx, y, dst, ncols, nrows, item_ct1, partial_sums);
            });
    });
#else
    const sycl::range<3> block_dims(1, ny, GGML_SYCL_DMMV_K_SUBGRP_SIZE);
