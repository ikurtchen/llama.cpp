...
// Template parameter NC enables compile-time specialization of the convolution
// kernel size (d_conv). When NC > 0, loops are fully unrolled and modulo
// becomes a compile-time operation. NC == 0 is the generic fallback.
template <int NC>
static void ssm_conv_f32_sycl(
    ...
    const int actual_nc = (NC > 0) ? NC : (int)nc;
    ...
    stream.parallel_for(
        ...
        [=](sycl::nd_item<2> item) [[intel::reqd_sub_group_size(WARP_SIZE)]] {
            ...
            // Private circular buffer and weights
            constexpr int buf_nc = (NC > 0) ? NC : SSM_CONV_MAX_D_CONV;
            float x[buf_nc] = {};
            float w[buf_nc] = {};

            // Load convolution weights
            #pragma unroll
            for (int j = 0; j < actual_nc; j++) {
            ...
            for (int64_t i = 0; i < n_t; i++) {
                ...
                if (i == 0) {
                    ...
                    x[(i - 1) % actual_nc] = x_block[tid * stride_x + i + actual_nc - 1];
                }

                #pragma unroll
                for (int j = 0; j < actual_nc; j++) {
                    sumf += x[(i + j) % actual_nc] * w[j];
                }
...
void ggml_sycl_ssm_conv(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    // Template dispatch macro to avoid repeating the long argument list.
    // Specializes for common d_conv values (4 for Mamba-1/2, 3 for some variants).
    // Falls back to generic NC=0 path for other values.
#define DISPATCH_SSM_CONV(kernel_fn)                       \
    do {                                                   \
        switch (nc) {                                      \
            case 4:                                        \
                kernel_fn<4>(src0_d, src1_d,               \
    ...
    if (n_t <= SSM_CONV_SPLIT_N_T) {
        DISPATCH_SSM_CONV(ssm_conv_f32_sycl);
    } else {
        DISPATCH_SSM_CONV(ssm_conv_long_token_f32_sycl);
    }

#undef DISPATCH_SSM_CONV
}
