...
// === COMB kernel: 4x4 combination matrix with softmax + Sinkhorn normalization ===
...
static void dsv4_hc_comb_kernel(
    ...
    // Sinkhorn iteration: normalize cols then rows, repeat
...
// === PRE kernel: per-head weighted sum across HC dimension ===
...
// === POST kernel: fused residual + combination-weighted sum ===
    ...
    stream->parallel_for(
        ...
        [=](sycl::nd_item<1> item) {
            dsv4_hc_pre_kernel(
                ...
                nbd0 / sizeof(float), nbd1 / sizeof(float), item);
        });
}
    ...
    stream->parallel_for(
        ...
        [=](sycl::nd_item<1> item) {
            dsv4_hc_post_kernel(
                ...
                nbd0 / sizeof(float), nbd1 / sizeof(float), nbd2 / sizeof(float), item);
        });
}