...
static void conv2d_transpose_kernel(const float * __restrict__ input, const sycl::half * __restrict__ kernel,
    ...
    for (int c_in_idx = 0; c_in_idx < c_in; c_in_idx++) {
        for (int kh = 0; kh < kernel_h; ++kh) {
            ...
            for (int kw = 0; kw < kernel_w; ++kw) {
                ...
                sycl::half kern_val = kernel[kernel_idx];

                accumulator += input_val * static_cast<float>(kern_val);
            }
        }
    }
...
void ggml_sycl_op_conv_transpose_2d(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    stream->parallel_for(
        sycl::nd_range<1>(sycl::range<1>(num_blocks) * sycl::range<1>(SYCL_CONV2D_TRANSPOSE_BLOCK_SIZE),
                          sycl::range<1>(SYCL_CONV2D_TRANSPOSE_BLOCK_SIZE)),
        [=](sycl::nd_item<1> item) {
            conv2d_transpose_kernel(input_data, kernel_data, output_data, input_w, input_h, output_w, output_h,
                                    kernel_w, kernel_h, stride, channels_in, channels_out, batches, item);
        });
}
