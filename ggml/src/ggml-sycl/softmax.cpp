...
template <typename T, bool use_shared>
    ...
    stream.submit([&](sycl::handler & h) {
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                ...
                max_val = block_reduce<block_reduce_method::MAX, 0>(max_val, buf_iw, item);
                if (tid == 0) {
                    buf_iw[0] = max_val;
                }
                ...
                tmp = block_reduce<block_reduce_method::SUM, 0>(tmp, buf_iw, item);
...
void ggml_sycl_op_soft_max(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    if (use_f16) {
        if (use_shared) {
            soft_max_f32_sycl<sycl::half, true>(src0_d, (const sycl::half *) src1_d, src2_d, dst_d,
                ne00, ne01, ne02, ne03, nb11, nb12, nb13, ne12, ne13,
                scale, max_bias, n_head_log2, m0, m1, stream);
        } else {
            soft_max_f32_sycl<sycl::half, false>(src0_d, (const sycl::half *) src1_d, src2_d, dst_d,
                ne00, ne01, ne02, ne03, nb11, nb12, nb13, ne12, ne13,
                scale, max_bias, n_head_log2, m0, m1, stream);
        }
    } else {
        if (use_shared) {
            soft_max_f32_sycl<float, true>(src0_d, (const float *) src1_d, src2_d, dst_d,
                ne00, ne01, ne02, ne03, nb11, nb12, nb13, ne12, ne13,
                scale, max_bias, n_head_log2, m0, m1, stream);
        } else {
            soft_max_f32_sycl<float, false>(src0_d, (const float *) src1_d, src2_d, dst_d,
                ne00, ne01, ne02, ne03, nb11, nb12, nb13, ne12, ne13,
                scale, max_bias, n_head_log2, m0, m1, stream);
        }
    }
