...
static inline dw_kernel_bounds calculate_dw_kernel_bounds(int32_t out_x, int32_t out_y, const conv2d_dw_params & P) {
...
static inline int32_t calculate_dw_input_coord(int32_t out_coord, int32_t kern_coord,
...
static inline int64_t whcn_input_index(int32_t n, int32_t c, int32_t y, int32_t x, const conv2d_dw_params & P) {
    return n * (P.C * P.IW * P.IH) + c * P.IW * P.IH + y * P.IW + x;
}

static inline int64_t whcn_kernel_index(int32_t c, int32_t ky, int32_t kx, const conv2d_dw_params & P) {
    return c * P.KH * P.KW + ky * P.KW + kx;
}

static inline int64_t whcn_output_index(int32_t n, int32_t c, int32_t y, int32_t x, const conv2d_dw_params & P) {
...
static inline int64_t cwhn_input_index(int32_t n, int32_t c, int32_t y, int32_t x, const conv2d_dw_params & P) {
    return n * (P.C * P.IW * P.IH) + (y * P.IW + x) * P.C + c;
}

static inline int64_t cwhn_kernel_index(int32_t c, int32_t ky, int32_t kx, const conv2d_dw_params & P) {
    return (ky * P.KW + kx) * P.C + c;
}

static inline int64_t cwhn_output_index(int32_t n, int32_t c, int32_t y, int32_t x, const conv2d_dw_params & P) {
...
static void conv2d_dw_kernel(const float * __restrict__ input,
    ...
    const int idx = (int) global_idx;
    int32_t n, c, out_y, out_x;
    if constexpr (is_cwhn) {
        cwhn_unpack_indices(idx, P, n, c, out_y, out_x);
    } else {
        whcn_unpack_indices(idx, P, n, c, out_y, out_x);
    }
    ...
    for (int32_t ky = bounds.y_min; ky < bounds.y_max; ++ky) {
        const int32_t in_y = calculate_dw_input_coord(out_y, ky, P.ST_Y, P.DL_Y, P.PD_Y);

        // opt_task_015: Enable unroll for kernel width loop
        // See: hw_spec_b60.md, optimization_guide
        #pragma unroll
        for (int32_t kx = bounds.x_min; kx < bounds.x_max; ++kx) {
            const int32_t in_x = calculate_dw_input_coord(out_x, kx, P.ST_X, P.DL_X, P.PD_X);
...
void ggml_sycl_op_conv_2d_dw(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    GGML_ASSERT(total <= INT32_MAX && "conv2d_dw TOTAL exceeds int32 range");
