...
void ggml_sycl_op_set(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    if (!inplace) {
        // ggml_sycl_cpy copies FROM first arg TO second arg
        ggml_sycl_cpy(ctx, src0, dst);
    }

    // Create a view into dst at the specified offset, with src1's shape and custom strides
    ...
    // Copy src1 into the view region of dst
